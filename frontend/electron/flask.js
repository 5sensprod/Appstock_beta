...
// Fonction pour vérifier si le port 5001 est déjà utilisé
...
function startFlask() {
  const flaskExecutablePath =
    process.env.NODE_ENV === 'development'
      ? path.join(__dirname, '../../backend/dist/app.exe') // Chemin en développement
      : path.join(process.resourcesPath, 'backend', 'app.exe') // Chemin en production (après empaquetage)

  console.log(`Démarrage de Flask à partir de ${flaskExecutablePath}...`)
  ...
  flaskProcess.on('error', (error) => {
    console.error(`Erreur lors du démarrage de Flask : ${error.message}`)
  })
...
function stopFlask() {
  if (flaskProcess) {
    ...
    treeKill(flaskProcess.pid, 'SIGKILL', (err) => {
      if (err) {
        console.error(`Erreur lors de l'arrêt du serveur Flask : ${err.message}`)
      } else {
    ...
    console.log("Aucun processus Flask en cours d'exécution.")
  }
}
...
async function fetchLocalIp() {
  try {
    ...
    console.error("Impossible de récupérer l'IP locale du serveur Flask :", error.message)
  }
}
