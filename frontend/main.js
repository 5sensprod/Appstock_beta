...
app.whenReady().then(async () => {
  if (!isDev) {
    console.log(
      'Production : démarrage du serveur Flask et Express'
    )
    ...
    if (!inUse) {
      startFlask()
      console.log(
        'Serveur Flask démarré sur le port 5000'
      )
    } else {
      console.log(
    ...
    waitForServer(() => {
      console.log(
        'Le serveur est prêt. Création de la fenêtre Electron'
      )
